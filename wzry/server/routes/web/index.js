module.exports = app => {
    ...
    router.get('/news/init', async(req, res) => {
        ...
        const cats = await Category.find().where({
            parent: parent
        }).lean() //lean()取出纯粹的json js对象和数组
        const newsTitles = ['微博电竞全明星公益赛收官之战来临！', '元歌皮肤设计大赛精彩创意赏析第六期', '偶像歌手登场 | 初次公演开始，抽水晶抽皮肤', '狄某有话说 | 但凡中个技能也不至于输出3%', '吕布-末日机甲海报场景票选活动开启公告', '1月14日体验服停机更新公告', '1月14日全民电竞主题赛异常说明', '1月13日全服不停机更新公告', '1月12日净化游戏环境声明及处罚公告', '1月12日外挂专项打击公告', '元歌皮肤设计大赛精彩创意赏析第六期', '“一”启幸运活动开启，概率得史诗皮肤', '吕布-末日机甲海报场景票选活动开启公告', '关羽-武圣返场！完成英雄挑战得英雄及全新荣耀播报！', '元歌皮肤设计大赛精彩创意赏析第五期', '挑战者杯预报丨秋决重演！武汉eStar战广州TTG谁能捧杯', '广州TTG夺得2022第三届王者荣耀微博杯总冠军', '候选人公布！年度最佳阵容最佳选手即将揭晓', '胡夏助力中场秀，2021挑战者杯总决赛节目单精彩出炉', '武汉ES、广州TTG顶峰再遇，挑战者杯总决赛1月15日开战']
    ...
    router.get('/news/list', async(req, res) => {
        ...
        const cats = await Category.aggregate([  //Mongodb聚合查询
            {$match: { parent: parent._id}}, //通过match过滤数据
            {
                $lookup:{ //lookup关联查询
                    from: 'articles',
                    localField:'_id',
            ...
            {
                $addFields: {
                    newsList:{$slice:['$newsList',5]} //截取5条
                }
            }
        ])
        //热门分类
        const subCats = cats.map(v => v._id) //subCats存放（新闻公告活动赛事）4个分类的id
        cats.unshift({
            name:'热门',
            newsList: await Article.find().where({//从文章集合中找出categories=4个分类id的所有文档
    ...
    router.get('/heroes/list', async(req, res) => {
        ...
        //热门分类
        const subCats = cats.map(v => v._id) //subCats存放英雄的6个分类的id
        cats.unshift({
            name:'热门',
            heroList: await Hero.find().where({//从英雄集合中找出categories=6个分类id的所有文档
                categories: {$in: subCats}
            }).limit(10).lean()
        })
